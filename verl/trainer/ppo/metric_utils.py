...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    rule_reward = batch.batch["rule_score"].sum(-1)
    rm_num = 0
    try:
        while True:
            batch.batch[f"rm_score_{rm_num}"]
            rm_num += 1
    except:
        print(rm_num)
    rm_rewards = [batch.batch[f"rm_score_{i}"].sum(-1) for i in range(rm_num)]

    advantages = batch.batch["advantages"]
    returns = batch.batch["returns"]
    try:
        bge_score = batch.batch["bge_score"].sum(-1)
    except:
        bge_score = None
    ...
    metrics = {
        ...
        "critic/rule_rewards/mean": torch.mean(rule_reward).detach().item(),

        # "critic/bge_rewards/mean": torch.mean(bge_score).detach().item(),
        # "critic/bge_rewards/max": torch.max(bge_score).detach().item(),
        # "critic/bge_rewards/min": torch.min(bge_score).detach().item(),
        # rm reward
        # "critic/rm_rewards/mean": torch.mean(rm_rewards).detach().item(),
        # "critic/rm_rewards/max": torch.max(rm_rewards).detach().item(),
        # "critic/rm_rewards/min": torch.min(rm_rewards).detach().item(),
        # adv
    ...
    if bge_score is not None:
        metrics["critic/bge_rewards/mean"] = torch.mean(bge_score).detach().item()
        metrics["critic/bge_rewards/max"] = torch.max(bge_score).detach().item()
        metrics["critic/bge_rewards/min"] = torch.min(bge_score).detach().item()
        metrics[f"critic/bge_rewards/var"] = torch.var(bge_score).detach().item() 
        
    for i in range(rm_num):
        metrics[f"critic/rm_rewards_{i}/mean"] = torch.mean(rm_rewards[i]).detach().item()
        metrics[f"critic/rm_rewards_{i}/max"] = torch.max(rm_rewards[i]).detach().item()
        metrics[f"critic/rm_rewards_{i}/min"] = torch.min(rm_rewards[i]).detach().item()
        metrics[f"critic/rm_rewards_{i}/var"] = torch.var(rm_rewards[i]).detach().item() 
