...
def find_list_lengths_omegaconf(cfg, lengths=None):
    if lengths is None:
        lengths = []
    if OmegaConf.is_dict(cfg):
        for v in cfg.values():
            find_list_lengths_omegaconf(v, lengths)
    elif OmegaConf.is_list(cfg):
        lengths.append(len(cfg))
        for item in cfg:
            find_list_lengths_omegaconf(item, lengths)
    return lengths

def check_list_lengths_omegaconf(cfg):
    lengths = find_list_lengths_omegaconf(cfg)
    if not lengths:
        return  # 没有列表
    if any(l != lengths[0] for l in lengths):
        raise ValueError("存在长度不一致的列表！")

def split_dict_omegaconf(cfg, idx):
    if OmegaConf.is_dict(cfg):
        return {k: split_dict_omegaconf(v, idx) for k, v in cfg.items()}
    elif OmegaConf.is_list(cfg):
        return split_dict_omegaconf(cfg[idx], idx)
    else:
        return cfg

def expand_omegaconf(cfg):
    check_list_lengths_omegaconf(cfg)
    lengths = find_list_lengths_omegaconf(cfg)
    if not lengths:
        return [cfg.copy()]  # 没有列表，直接返回原 config 的拷贝
    n = lengths[0]
    result = []
    for i in range(n):
        item = split_dict_omegaconf(cfg, i)
        # 包装成 DictConfig
        result.append(OmegaConf.create(item))
    return result
...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    """
    if data_source == "openai/gsm8k" or '/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtai/users/zhangrunlai/data/GSM8k' in data_source:
        from . import gsm8k

        res = gsm8k.compute_score(solution_str, ground_truth)
    else:
        from . import rider_agent_reason_v2
        res = rider_agent_reason_v2.compute_score(solution_str, ground_truth)
