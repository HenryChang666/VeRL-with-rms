...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        with _timer("reshard", self.timing):
            ...
            print(f"before reload fsdp policy memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
            # breakpoint()
            peft_config = None
            peft_model = getattr(self.module, "_fsdp_wrapped_module", self.module)
            ...
            print(f"before rebuild policy vllm memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
            print("self.full_params is " + str(self.full_params))
            print("hasattr(peft_model, 'peft_config')" + str(hasattr(peft_model, "peft_config")))
            ...
            if vllm_version in (
                ...
                print(f"after rebuild policy vllm memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
                del params
            else:
...
class VLLMRewardModelShardingManager(BaseShardingManager):
    """
    这个其实可以删掉直接用 FSDPVLLMShardingManager 也一样。重新开一个 Manager 原本是为了 vllm V1，但是 V1 版本的已经废弃了。
    """
    @check_device_is_available()
    def __init__(self, module: FSDP, inference_engine: LLM, device_mesh: DeviceMesh = None, full_params: bool = False, load_format: str = "dummy_hf"):
        self.module = module
        # For AsyncLLM, inference_engine and model_runner are defer initialized in vLLMAsyncRollout.load_model
        self.inference_engine = inference_engine
        # self.model_runner = inference_engine.llm_engine.model_executor.driver_worker.worker.model_runner if inference_engine else None
    ...
    def __enter__(self):
        ...
        with _timer("reshard", self.timing):
            ...
            log_gpu_memory_usage("Before state_dict() in sharding manager memory", logger=logger)
            load_fsdp_model_to_gpu(self.module)
            print(f"before reload fsdp reward memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
            ...
            load_format = "hf" if self.full_params else "dtensor"
            # breakpoint()
            print(f"before rebuild reward vllm memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
            ...
            if vllm_version in (
                ...
                self.inference_engine.sync_model_weights(params, load_format=load_format)
                log_gpu_memory_usage("After sync model weights in sharding manager", logger=logger)
                print(f"after rebuild reward vllm memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
                del params
                offload_fsdp_model_to_cpu(self.module)
                get_torch_device().empty_cache()
                print(f"after offload fsdp reward memory allocated: {torch.cuda.memory_allocated() / 1e9}GB, reserved: {torch.cuda.memory_reserved() / 1e9}GB")
            else:
                if "tags" in inspect.signature(self.inference_engine.wake_up).parameters:
                    self.inference_engine.wake_up(tags=["weights"])
                else:
                ...
                log_gpu_memory_usage("After wake up reward model weights in sharding manager", logger=logger)           
