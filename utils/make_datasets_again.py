...
if __name__ == "__main__":
    ...
    parser.add_argument("--prob", type=float, default=0.89, help="number of samples per task")
    args = parser.parse_args()
    
    args.input = "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtai/users/zhangxiaoyun15/program/voc_explore/.tmp/DeepSeek-R1-Distill-Qwen-14B/traindata_voc_hard_filtered_19457_r1_formatted_stage_100.json"
    args.output = "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtai/users/zhangxiaoyun15/program/voc_explore/data/train/DeepSeek-R1-Distill-Qwen-14B/traindata_voc_hard_filtered_19457_r1_formatted_stage_100.json"
    ...
    ##### Counter({10: 14058, 9: 1842, 8: 775, 7: 513, 0: 443, 6: 411, 5: 317, 2: 293, 4: 272, 3: 269, 1: 264})
    ...
    sample_size = 2000
    # 计算每个组的比例
    total_items = sum(len(items) for items in grouped_outputs.values())
    sample_counts = {pass_value: min(len(items), int(len(items) / total_items * sample_size)) for pass_value, items in grouped_outputs.items()}

    for pass_value, items in grouped_outputs.items():
        sample_count = sample_counts[pass_value]
