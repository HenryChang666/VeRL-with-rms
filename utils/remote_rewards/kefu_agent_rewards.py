...
def reward_fn(query:str):
    ...
    try:
        ...
        # 字段缺失
        if any([x not in pred_js for x in ['Response', 'ResponseSolution', 'DialogueAgreeSolution']]):
            return 0.0
        # 话术分数（0-1）
        response_score = calc_response_score(pred_js['Response'], target['Response'], context)
        # 回复方案分数(0.1 / 1)
        solution_score = 1.0 if (pred_js['ResponseSolution'] == target['ResponseSolution'] and pred_js['DialogueAgreeSolution'] == target['DialogueAgreeSolution']) else 0.1
        return response_score * solution_score
    except:
        # 输出不合法
        return 0.0
